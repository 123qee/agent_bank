...
LLM_PARSE_ENABLED = os.getenv("ONE_API_PARSE", "1").lower() in ("1", "true", "yes")
...
def _norm_rate(value: Any) -> Optional[float]:
...
def _norm_int(value: Any, min_value: int, max_value: int) -> Optional[int]:
    if value is None or value == "":
        return None
    try:
        num = int(float(value))
...
def _validate_llm_task(obj: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    allowed_fields = {
        "intent", "user_id", "life_expectancy", "inflation", "investment_return",
        "split_after_years", "later_inflation", "allocation_mode", "product_preference",
        "remember",
    }
    task = {k: obj.get(k) for k in allowed_fields if k in obj}
    intent = str(task.get("intent") or "").strip().strip("`'\"").lower()
    ...
    if user_id:
        m = re.search(r"V\d{6,}", str(user_id), re.I)
        if m:
            task["user_id"] = m.group(0).upper()
        else:
            task.pop("user_id", None)

    life = _norm_int(task.get("life_expectancy"), 50, 120)
    if life is not None:
        task["life_expectancy"] = life
    else:
        task.pop("life_expectancy", None)

    split_years = _norm_int(task.get("split_after_years"), 0, 80)
    if split_years is not None:
        task["split_after_years"] = split_years
    else:
        task.pop("split_after_years", None)

    for key in ("inflation", "investment_return", "later_inflation"):
        rate = _norm_rate(task.get(key))
        if rate is not None:
            task[key] = rate
        else:
            task.pop(key, None)

    allocation_mode = task.get("allocation_mode")
    if allocation_mode not in ("max_return", "low_vol", None, ""):
        task.pop("allocation_mode", None)
    elif allocation_mode in (None, ""):
        task.pop("allocation_mode", None)

    product = task.get("product_preference")
    if product:
        product_text = str(product)
        matched = None
        for name in PRODUCT_NAMES:
            if name in product_text or fmt_product_name(name) in product_text:
                matched = name
                break
        if matched:
            task["product_preference"] = matched
        else:
            task.pop("product_preference", None)

    task["remember"] = bool(task.get("remember"))
    return task


def llm_parse_task(text: str) -> Optional[Dict[str, Any]]:
    if not LLM_ENABLED or not LLM_FALLBACK or not LLM_PARSE_ENABLED:
        debug_log(
            "llm_call_skipped",
            call_site="llm_parse_task",
            purpose="规则路由未命中时，用大模型抽取结构化任务参数",
            reason="LLM disabled or parse disabled",
            question=text,
        )
        return None
    system = (
        "你是养老规划Agent的任务解析器。请把客户经理问题解析成一个 JSON 对象，"
        "只输出 JSON，不要解释、不要 Markdown。允许字段仅有："
        "intent,user_id,life_expectancy,inflation,investment_return,split_after_years,"
        "later_inflation,allocation_mode,product_preference,remember。"
        "intent 只能从以下标签中选择：" + ",".join(INTENT_LABELS) + "。"
        "通胀率、投资回报率、later_inflation 用小数表示，例如 3% 输出 0.03。"
        "allocation_mode 只能是 max_return 或 low_vol。"
        "product_preference 只能是现金理财、定期存款、短债类产品、固收+产品、权益类产品、年金险之一。"
        "如果是客户明确观点/偏好且应影响后续问题，remember 为 true；如果是假如/假设/如果等临时场景，remember 为 false。"
        "无法判断的字段用 null。"
    )
    prompt = f"问题：{text}\n输出 JSON："
    res = llm_chat(
    ...
    obj = _extract_json_object(res or "")
    task = _validate_llm_task(obj) if obj else None
    debug_log("llm_task", question=text, raw=res, task=task)
    return task
...
def direct_field_answer(text: str, user: Dict[str, Any]) -> Optional[str]:
    ...
    if "净资产" in text:
        return fmt_short_money(money(user.get("Net_Asset")))
    if "月收入" in text or "每月收入" in text or "收入多少" in text:
        return fmt_short_money(money(user.get("Monthly_Income")))
    if ("月支出" in text or "每月支出" in text or "开销" in text) and "退休" not in text:
        return fmt_short_money(money(user.get("Monthly_Expend")))
    if "结余" in text:
        return fmt_short_money(monthly_surplus(user))
    if "企业年金" in text:
        value = money(user.get("Enterprise_Ann"))
        return "无" if value <= 0 else fmt_short_money(value)
    if ("退休金" in text or "养老金" in text) and not any(
        ...
        return fmt_short_money(money(user.get("Pension")))
    return None
...
def _answer_by_intent(
    ...
    if intent == "future_expend":
        return fmt_short_money(future_monthly_expend(user, ctx_inflation(ctx), ctx.get("split_after_years"), ctx.get("later_inflation")))
...
def answer_user_question(text: str, user_id: str, user: Dict[str, Any]) -> str:
    ...
    # 规则没匹配上时，优先让大模型抽取结构化参数，再交给本地 skill 确定性计算。
    parsed_task = llm_parse_task(text)
    if parsed_task:
        parsed_ctx = dict(ctx)
        for key in (
            "life_expectancy", "inflation", "investment_return", "split_after_years",
            "later_inflation", "allocation_mode", "product_preference",
        ):
            if key in parsed_task:
                parsed_ctx[key] = parsed_task[key]
        if parsed_task.get("remember"):
            with _session_lock:
                remembered = {
                    key: parsed_ctx[key]
                    for key in (
                    ...
                    if key in parsed_task and key in parsed_ctx
                }
                if remembered:
                    _session_memory.setdefault(user_id, {}).update(remembered)
        ans = _answer_by_intent(str(parsed_task["intent"]), text, user_id, user, parsed_ctx)
        if ans is not None:
            _remember_discussed_intent(user_id, str(parsed_task["intent"]))
            debug_log("answer", user_id=user_id, intent=f"llm_parse:{parsed_task['intent']}", answer=ans)
            return ans

    # 结构化解析失败时，退回原来的标签分类；答案仍由代码确定性计算。
